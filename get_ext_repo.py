                ...
                try:
                    ...
                    if returned.status_code == 201:
                        if data["ztp"]:
                            ...
                            rd.hmset(
                                serialNumber, {"hostname": f"{serialNumber} - [ZTP]"}
                            )
                            ...
                            for item in url_list:
                                ...
                                rd.hmset(
                                    serialNumber, {f"device_repo_{item}": f"{url}"}
                                )
                        ...
                        and "this name already exists" in returned.text
                    ):
        ...
        try:
            ...
            for x in retuned:
                ...
                if x["path_with_namespace"] in ext_url:
                    ...
                    r = requests.get(
                        "{0}/api/v4/projects/{1}/repository/tree".format(
                            gitlab_url, x["id"]
                        )
                    )
                    d = r.json()
                    ...
                    for path in d:
                        if ".j2" in path["path"]:
                            if "all" in args:
                                ext_repo_files[
                                    path["path"]
                                ] = "{0}/raw/master/{1}".format(ext_url, path["path"])
                                # print('---- ALL ----')
                            else:
